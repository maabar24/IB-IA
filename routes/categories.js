...
categoriesRouter.post('/', requiresAuth(), (req, res) => {
    db.execute(create_categories_sql, [req.body.category_name, req.oidc.user.sub], (error, results) =>{
        if (DEBUG)
            console.log(error ? error : results);
        if (error)
            res.status(500).send(error); //Internal Server Error
    ...
    db.execute(delete_categories_sql, [req.params.id], (error, results) => {
        ...
        if (error){
            ...
            if (error.code == "ER_ROW_IS_REFERENCED_2"){
                res.status(500).send("There are foods still associated with that categories!")
            }
            else 
                res.status(500).send(error); //Internal Server Error
        }
