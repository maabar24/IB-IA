        ...
        if (error) {
            ...
            db.execute(read_categories_all_sql, (error2, results2) => {
                ...
                if (error2)
                    ...
                    let data = {ingredient: results, categories: results2};
...
listRouter.get( "/list/edit/:inventory_id", ( req, res ) => {
    db.execute(read_ingredient_sql, [req.params.id, req.oidc.user.email], (error, results) => {
        if (error) {
            res.status(500).send(error); //internal server error
        } 
            ...
            //res.send(results[0]);
            res.render('edit', { ingredient: results[0] }); //results[0]
        }
...
listRouter.get("/list/edit/:inventory_id/delete", ( req, res ) => {
    db.execute(delete_ingredient_sql, [req.params.id, req.oidc.user.email], (error, results) => {
        if (error)
            res.status(500).send(error); //Internal Server Error
        else {
            res.redirect("/");
...
listRouter.post("/", ( req, res ) => {
    db.execute(create_ingredient_sql, [req.body.name, req.body.price, req.body.quantity, req.body.categories, req.oidc.user.email], (error, results) => {
        if (error)
            res.status(500).send(error); //Internal Server Error
        else {
...
listRouter.post("/list/edit/:inventory_id", ( req, res ) => {
    db.execute(update_ingredient_sql, [req.body.name, req.body.quantity, req.body.price, req.body.categories, req.params.id, req.oidc.user.email], (error, results) => {
        if (error)
            res.status(500).send(error); //Internal Server Error
        else {
